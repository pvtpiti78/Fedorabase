...
# =============================================================================
# Ausgangslage: Minimale Fedora 44 TTY-Installation, AMD Radeon GPU (amdgpu)
# Umfang: DNF5-Tuning, RPM Fusion, Mesa/Vulkan (AMD), Fish, Kitty, Starship,
#         Fastfetch, Firefox, Steam, ProtonPlus, Faugus, LACT, amd.conf, gaming.conf
# =============================================================================
...
echo "  ╚═╝     ╚══════╝╚═════╝  ╚═════╝ ╚═╝  ╚═╝╚═╝  ╚═╝"
echo -e "${NC}"
echo -e "  ${BOLD}Fedora 44 — Base Setup (AMD)${NC}"
echo -e "  amdgpu/Mesa · Fish · Kitty · Starship · Gaming ENV"
echo ""
...
# ── AMD Grafikstack (Mesa/Vulkan/VA-API) ─────────────────────────────────────
# amdgpu ist bereits im Kernel — kein Treiber-Install, kein DKMS, kein Reboot
# zum Laden eines Kernelmoduls nötig. Nur Userspace-Stack auffrischen/sicherstellen.
info "Mesa/Vulkan-Stack (AMD) installieren..."
dnf install -y \
    mesa-vulkan-drivers \
...
# ── LACT (AMD Undervolting/Overclocking) ─────────────────────────────────────
# Overdrive-Freischaltung (Voltage/Power-Limit-Schreibzugriff) erfolgt NICHT
# hier per Kernel-Cmdline, sondern bewusst über den "AMD Overclocking aktivieren"
# Button in der LACT-GUI selbst (schreibt /etc/modprobe.d/99-amdgpu-overdrive.conf
# und baut das initramfs neu). Nach der Installation einmal in LACT aktivieren
# und neustarten.
info "LACT installieren..."
dnf copr enable -y ilyaz/LACT
dnf install -y lact || true
systemctl enable --now lactd
log "LACT installiert (Overdrive-Freischaltung über LACT-GUI aktivieren, dann Reboot)"
...
### FSR4 — Proton-DLL-Swap (RDNA4 native FP8-Pfad)
PROTON_FSR4_UPGRADE=1

### VKD3D — Descriptor Heap (experimenteller Codepfad)
...
# ── amd.conf ENV (Wayland/Vulkan/VA-API) ─────────────────────────────────────
...
echo -e "  • LACT öffnen und 'AMD Overclocking aktivieren' klicken, dann neustarten"
echo ""
echo -e "  ${YELLOW}Neustart empfohlen (NTSYNC-Modul, sysctl, ENV-Dateien greifen vollständig erst danach)${NC}"
echo ""
